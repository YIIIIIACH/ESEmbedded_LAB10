...
void set_mpu(void)
{
	...
	WRITE_BITS( MPU_BASE+MPU_RASR_OFFSET, 31,0 ,MPU_RASR_VALUE(MPU_XN_DISABLE, MPU_AP_FULL_ACCESS, MPU_TYPE_SRAM , 0, MPU_REGION_SIZE_128KB));
	...
	for (int i =4; i<= 7; i++)
	{
		REG(MPU_BASE+MPU_RBAR_OFFSET) = MPU_RBAR_VALUE( 0x80000000+i*4, i);
